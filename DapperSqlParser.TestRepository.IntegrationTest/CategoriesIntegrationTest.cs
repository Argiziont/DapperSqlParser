...
namespace DapperSqlParser.TestRepository.IntegrationTest
{
    public class CategoriesIntegrationTest
    {
        ...
        public async Task Delete_EndpointsReturnSuccessAndCorrectContentType(string url, int categoryId)
        {
            ...
            //Act
            var response = await client.DeleteAsync($"{url}?categoryId={categoryId}");
            response.EnsureSuccessStatusCode();
        ...
        public async Task Update_EndpointsReturnSuccessAndCorrectContentType(string url, int categoryId, string categoryTitle)
        {
            ...
            //Act
            var response = await client.PutAsync($"{url}?categoryId={categoryId}", new StringContent(categoryTitle, System.Text.Encoding.UTF8, "application/json"));
            response.EnsureSuccessStatusCode();
