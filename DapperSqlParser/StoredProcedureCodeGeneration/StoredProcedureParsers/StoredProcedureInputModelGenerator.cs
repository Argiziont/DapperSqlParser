...
namespace DapperSqlParser.StoredProcedureCodeGeneration.StoredProcedureParsers
{
    public class StoredProcedureInputModelGenerator:ICodeGenerator
    {
        ...
        public StoredProcedureInputModelGenerator(InputParametersDataModel[] inputModels, string storedProcedureName, string storedProcedureDefinition)
        {
        ...
        private async Task<string> CreateSpDataModelForInputParams()
        {
            ...
            if (CheckStringForInputJsonSnippets(_storedProcedureDefinition))
                return $"{CodeGeneratorUtils.CreateJsonWrapperClassAttribute(_inputModels.First().ParameterName)} {await CreateSpDataModelForInputParamsJson(_storedProcedureDefinition)}";
            ...
            foreach (InputParametersDataModel field in _inputModels)
                inputClass.AppendLine(CodeGeneratorUtils.CreateProperty(
                    ...
                    (field.ParameterName == null ? "Result" : $"{field.ParameterName.Replace("-", "_").Replace("@", "").FirstCharToUpper()}"),//Append property name
                    CodeGeneratorUtils.CreateJsonPropertyAttribute(field.ParameterName.Replace("@", ""), field.IsNullable)//Append property attribute
                ));
